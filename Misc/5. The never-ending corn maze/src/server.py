...
def index():
    ...
    large_maze = maze.get_maze(300, 300, seed)
    solution = solver.solve_maze(large_maze)

    hasher_thread = Thread(target=get_and_store_hash, kwargs={
                           'maze': large_maze, 'solution': solution})
    hasher_thread.start()
